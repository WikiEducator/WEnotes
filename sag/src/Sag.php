...
class Sag {
  ...
  /**
     * PUTs the provided document. When using a SagCache, the created document
     * and response are not cached.
     *
     * @param mixed $data The document that you want created. Can be an object,
     * array, or string.
     ...
     * @return mixed
     */
  public function putNew($data, $path = null) {
    if(!$this->db) {
      throw new SagException('No database specified');
    }
    ...
    //return $this->procPacket('POST', "/{$this->db}{$path}", $data);
    return $this->procPacket('PUT', "/{$this->db}{$path}", $data);
  }
